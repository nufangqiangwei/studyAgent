...
```go
gateway, err := webgateway.NewModule(webgateway.ModuleOptions{Presenter: presenter, Clock: clock})
if err != nil {
    return err
}
...
成为 Task Owner。本模块不会自动发送 `task.mark_ready`、`task.assign` 或 `task.start`。
