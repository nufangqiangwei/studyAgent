...
func (t *ListFilesTool) Execute(ctx context.Context, input json.RawMessage) (Result, error) {
	if err := ctx.Err(); err != nil {
		return Result{}, err
	}
	if t == nil {
		return Result{}, errors.New("list_files: tool is nil")
	}
...
func (t *ReadFileTool) Execute(ctx context.Context, input json.RawMessage) (Result, error) {
	...
	if t == nil {
		return Result{}, errors.New("read_file: tool is nil")
	}
	...
	if err != nil {
		return Result{}, fmt.Errorf("read_file: marshal result: %w", err)
	}
...
func (t *SearchTextTool) Execute(ctx context.Context, input json.RawMessage) (Result, error) {
	if err := ctx.Err(); err != nil {
		return Result{}, err
	}
	if t == nil {
		return Result{}, errors.New("search_text: tool is nil")
	}
	...
	ws, err := workspaceFromContext(ctx)
	if err != nil {
		return Result{}, fmt.Errorf("search_text: %w", err)
	}
...
func (t *GetWorkspaceSummaryTool) Execute(ctx context.Context, input json.RawMessage) (Result, error) {
	if err := ctx.Err(); err != nil {
		return Result{}, err
	}
	if t == nil {
		return Result{}, errors.New("get_workspace_summary: tool is nil")
	}
	...
	ws, err := workspaceFromContext(ctx)
	if err != nil {
		return Result{}, fmt.Errorf("get_workspace_summary: %w", err)
	}
	if normalized.Path != "." {
	...
	if err != nil {
		return Result{}, fmt.Errorf("get_workspace_summary: marshal result: %w", err)
	}
...
func jsonToolResult(value any, metadata map[string]any) (Result, error) {
	...
	if err := json.Indent(&pretty, raw, "", "  "); err != nil {
		return Result{}, fmt.Errorf("format result: %w", err)
	}
	return Result{
