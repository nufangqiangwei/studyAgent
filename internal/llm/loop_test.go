...
func TestRuntimeBuildRequestMergesAgentProfileMessagesToolsAndMetadata(t *testing.T) {
	rt, err := NewRuntime(Options{LLM: &recordingLLM{}})
	if err != nil {
...
func TestRuntimeCallModelReturnsNormalizedResponse(t *testing.T) {
	...
	rt, err := NewRuntime(Options{LLM: model})
	if err != nil {
		t.Fatalf("NewRuntime returned error: %v", err)
	}
...
func TestRuntimeCallModelReturnsErrorWithoutResponseMutation(t *testing.T) {
	modelErr := errors.New("model failed")
	rt, err := NewRuntime(Options{LLM: &recordingLLM{err: modelErr}})
	if err != nil {
		t.Fatalf("NewRuntime returned error: %v", err)
	}
...
func TestRuntimeConcurrentCallsDoNotShareMutableState(t *testing.T) {
