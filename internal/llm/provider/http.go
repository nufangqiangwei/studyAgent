...
func (c httpClient) Complete(ctx context.Context, req llm.Request) (llm.Response, error) {
	if c.apiKey == "" {
		return llm.Response{}, fmt.Errorf("llm %s: api_key is required for model %q", c.provider, c.model)
	}
	...
	if err != nil {
		return llm.Response{}, fmt.Errorf("build %s request: %w", c.provider, err)
	}
	...
	if err != nil {
		if debugErr := c.recordDebug(ctx, startedAt, time.Now().UTC(), req, body, nil, fmt.Sprintf("create %s request: %v", c.provider, err)); debugErr != nil {
			return llm.Response{}, debugErr
		}
		return llm.Response{}, fmt.Errorf("create %s request: %w", c.provider, err)
	}
	...
	if err != nil {
		...
		return llm.Response{}, fmt.Errorf("send %s request: %w", c.provider, err)
	}
	...
	if err != nil {
		...
		return llm.Response{}, fmt.Errorf("read %s response: %w", c.provider, err)
	}
	...
	if resp.StatusCode < http.StatusOK || resp.StatusCode >= http.StatusMultipleChoices {
		...
		return llm.Response{}, fmt.Errorf("%s", errText)
	}
	...
	if err != nil {
		...
		return llm.Response{}, fmt.Errorf("parse %s response: %w", c.provider, err)
	}
...
func (c httpClient) recordDebug(ctx context.Context, startedAt, completedAt time.Time, req llm.Request, requestBody []byte, resp *http.Response, errorText string, responseBody ...byte) error {
	if c.debug == nil {
		return nil
	}
