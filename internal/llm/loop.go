...
func NewRuntime(opts Options) (*Runtime, error) {
	if opts.LLM == nil {
		return nil, fmt.Errorf("llm: llm client is required")
	}
	return &Runtime{llm: opts.LLM, logger: opts.Logger}, nil
}

func (r *Runtime) BuildRequest(_ context.Context, input ModelCallInput) (llmClient.Request, error) {
	if r == nil {
		return llmClient.Request{}, fmt.Errorf("llm: llm is nil")
	}
	if strings.TrimSpace(input.Agent.Model) == "" {
		return llmClient.Request{}, fmt.Errorf("llm: agent model is required")
	}
	if len(input.Messages) == 0 {
		return llmClient.Request{}, fmt.Errorf("llm: messages are required")
	}
...
func (r *Runtime) CallModel(ctx context.Context, input ModelCallInput) (ModelCallResult, error) {
	if r == nil {
...
func (r *Runtime) CompleteRequest(ctx context.Context, input ModelCallInput, request llmClient.Request) (ModelCallResult, error) {
	...
	if err != nil {
		return result, fmt.Errorf("llm llm complete: %w", err)
	}
