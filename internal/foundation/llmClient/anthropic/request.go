...
func (Builder) Build(req llmClient.Request) ([]byte, error) {
	...
	for _, msg := range req.Messages {
		...
		if msg.Role == llmClient.RoleTool {
...
func appendToolResult(messages []Message, msg llmClient.Message) []Message {
	block := ContentBlock{
		Type:      "tool_result",
...
func anthropicRole(role llmClient.Role) string {
	if role == llmClient.RoleAssistant {
		return "assistant"
	}
	return "user"
}

func anthropicContent(msg llmClient.Message) any {
...
func messageTools(defs []llmClient.ToolDefinition) []Tool {
	if len(defs) == 0 {
		return nil
	}
