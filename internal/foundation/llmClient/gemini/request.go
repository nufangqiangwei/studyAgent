...
func geminiRole(role llmClient.Role) string {
...
func geminiParts(msg llmClient.Message) []Part {
	switch {
	case msg.Role == llmClient.RoleTool:
		return []Part{{
...
func contentTools(defs []llmClient.ToolDefinition) []Tool {
	if len(defs) == 0 {
		return nil
	}
