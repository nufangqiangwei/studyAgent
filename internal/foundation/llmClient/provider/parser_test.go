...
func TestOpenAIParserPreservesToolCallID(t *testing.T) {
	resp, err := (openAIParser{provider: "openai"}).Parse(llmClient.Request{Model: "gpt-test"}, []byte(`{
  "model": "gpt-test",
  "choices": [{
...
func TestAnthropicParserPreservesToolUseID(t *testing.T) {
	resp, err := (anthropicParser{provider: "anthropic"}).Parse(llmClient.Request{Model: "claude-test"}, []byte(`{
  "model": "claude-test",
...
func TestGeminiParserPreservesFunctionCallID(t *testing.T) {
	resp, err := (geminiParser{}).Parse(llmClient.Request{Model: "gemini-test"}, []byte(`{
  "candidates": [{
    "finishReason": "STOP",
    "content": {
      "parts": [{
