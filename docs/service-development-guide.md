...
Service 不得取得其他 Service 的 Go 对象、Store、Activation 或内存状态。跨服务依赖表示逻辑地址和消息契约，不表示对象注入。
...
只有外部 callback 无法通过普通 Decision 表达时才使用 RuntimeBinder。Binder 只能取得通用 `RuntimePorts`，并把 callback 转成稳定、可序列化 Message 送入 Durable Inbox；不能取得 Host、Store 或 Service 对象。
